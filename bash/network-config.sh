...
host=$(hostname)
ifnmae=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
ladress=$(ip a s $ifname |awk '/inet /{gsub(/\/.*/,"");print $2}')
lanhost=$(getent hosts $ladress|awk '{print $2}')
exip=$(curl -s icanhazip.com)
extname=$(getent hosts $exip|awk '{print $2}')
